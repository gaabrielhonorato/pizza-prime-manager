...
import { Receipt, Clock, Send, CheckCircle, Ban, Eye, CalendarDays } from "lucide-react";
...
export default function FinanceiroCobrancas() {
  ...
  // Modal state
  const [genModal, setGenModal] = useState<string | null>(null); // pizzaria_id
  ...
  // Get pedidos already included in non-cancelled cobrancas
  ...
  // Calculate pending amounts per pizzaria
  ...
  const handleGenerate = async () => {
    ...
    const { error } = await supabase.from("cobrancas_repasse").insert({
      ...
      total_delivery: genPz.totalDel,
      total_retirada: genPz.totalRet,
      total_local: genPz.totalLoc,
      taxa_delivery_aplicada: taxaDel,
      taxa_retirada_aplicada: taxaRet,
      taxa_local_aplicada: taxaLoc,
      valor_automatico_pp: genPz.autoSplit,
      valor_manual_devido: genPz.totalPP - genPz.autoSplit,
      valor_total_devido: genPz.totalPP - genPz.autoSplit,
      data_agendada: dataAgendada,
      status,
      data_envio: sendOption === "agora" ? new Date().toISOString() : null,
      pedidos_snapshot: snapshot,
    });

    if (error) { toast.error("Erro ao gerar cobrança."); setSaving(false); return; }

    // If sending now, create WhatsApp dispatch
    if (sendOption === "agora") {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pizzarias with pending balance */}
      ...
      {/* Generated billings */}
      <Card className="border-border bg-card">
        <CardHeader><CardTitle className="font-heading">Cobranças geradas</CardTitle></CardHeader>
        <CardContent>
          {cobrancas.length === 0 ? (
